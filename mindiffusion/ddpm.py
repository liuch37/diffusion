...
class DDPM_Context(nn.Module):
    def __init__(
        ...
        super(DDPM_Context, self).__init__()
        self.eps_model = eps_model
    ...
    def forward(self, x: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
        ...
        return self.criterion(eps, self.eps_model(x_t, _ts / self.n_T, c))

    def sample(self, n_sample: int, size, device, c=None) -> torch.Tensor:
        ...
        for i in range(self.n_T, 0, -1):
            z = torch.randn(n_sample, *size).to(device) if i > 1 else 0
            eps = self.eps_model(
                x_i, torch.tensor(i / self.n_T).to(device).repeat(n_sample, 1), c
            )
            x_i = (
                self.oneover_sqrta[i] * (x_i - eps * self.mab_over_sqrtmab[i])
                + self.sqrt_beta_t[i] * z
